...
bool allow_low_power_mode(pm_t mode) {
    ...
    /* Escape hatch: refuse deep sleep entirely, so none of the peripheral teardown
       in lpm_enter_low_power() runs and the board simply never sleeps. Costs battery
       life; kept as a one-line way to rule the sleep path out if waking ever breaks
       again. Normally left undefined. */
